...
public class ArrayStack<T extends Number> implements CustomStack<T> {
    ...
    public ArrayStack() {
        ...
        operationCount = 0;
        stackId = ++totalStacks;  // Assign a unique ID
    }
    ...
    public T pop() {
        ...
        elements[top--] = null;  // Clear the reference for garbage collection
        totalElements--;
        return element;
    }
    ...
    public void addTopTwo() {
        if (size() < 2) {
            throw new IllegalStateException("Stack must have at least two elements to perform addition.");
        }
        T first = pop();
        T second = pop();
